function out = model_equisetum3_plot(n, par)
    % Unpack parameters from vector 
    qk      = par(1);        % 17O exponent (diffusion of water vapour through air)
    ea      = par(2);        % vapor pressure (air) 
    es      = par(3);        % vapor pressure at leaf surface
    T       = par(4);        % Temperature (celsius) 
    d18Omw  = par(5);        % d18O of source water (VSMOW scale)
    n = round(n);        
    ...
    R17_SMOW = 0.0003799;
    q17 = 0.529;
    ak      = 1.0350;        % fractionation factor for diffusion of water vapour through air
    akh     = ak^(2/3);      % kinetic fractionation factor for transport across boundary layer
    y       = 1.0;        % transpiration tuning exponent
